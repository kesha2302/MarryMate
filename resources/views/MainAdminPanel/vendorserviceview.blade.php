        ...
        <div class="container-fluid">
            ...
            <form class="d-flex" action="">
                ...
                <span style="margin-left: 10px;">
                    <a href="{{ url('/VendorsServicesDetail') }}">
                    ...
                    <tbody>
                        ...
                        @foreach ($services as $service)
                            <tr>
                                ...
                                @if (!in_array($service->vendor_id, $printedVendors))
                                    <td rowspan="{{ $services->where('vendor_id', $service->vendor_id)->count() }}">
                                        {{ $service->vendor->user->name ?? 'No Vendor' }}
                                    </td>
                                    <td rowspan="{{ $services->where('vendor_id', $service->vendor_id)->count() }}">
                                        @if ($service->status == 1)
                                    ...
                                    @php
                                        $printedVendors[] = $service->vendor_id;
                                    @endphp
                                @endif
                                ...
                                {{-- <td>
                                    <button class="btn btn-sm btn-success add-to-package"
